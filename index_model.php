...
class IndexModel
{
	...
	{
		$q = "SELECT *,UserTypeTitle,DepartmentName FROM users LEFT OUTER JOIN department on users.DepartmentID = department.DepartmentID LEFT OUTER JOIN usertype on users.UserTypeID= usertype.UserTypeID where users.UserTypeID !=3";
		$r = mysqli_query($this->con, $q);
		if ($r == false) {
			return 0;
		} else {
